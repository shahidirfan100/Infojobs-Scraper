...
await Actor.main(async () => {
    ...
    // --------- HANDSHAKE: PLAYWRIGHT TO GET COOKIES & UA ---------
    ...
    async function acquireSessionCookies(startUrlForHandshake, reason = 'initial') {
        ...
        const handshakeCrawler = new PlaywrightCrawler({
            ...
            async requestHandler({ page, request, log: crawlerLog }) {
                ...
                if (ua) {
                    userAgent = ua;
                    crawlerLog.info(`[HANDSHAKE] Using UA: ${userAgent}`);
                }
            },
    ...
    if (!sessionCookies.length) {
        log.warning(
            '[HANDSHAKE] No cookies after initial handshake – continuing anyway (site may still work).',
        );
    } else {
        log.info('[HANDSHAKE] Cookie handshake complete, switching to CheerioCrawler.');
    }

    // --------- HELPERS SHARED BY CHEERIO CRAWLER ---------
    ...
    function findJobLinks($, base, seen) {
        ...
        for (const sel of selectors) {
            ...
            if (links.size >= 15) break; // avoid over-enqueueing weird URLs
        }
    ...
    const crawler = new CheerioCrawler({
        ...
        useSessionPool: false, // We're managing cookies manually
